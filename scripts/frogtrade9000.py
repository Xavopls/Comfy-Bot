...
def setup_client(name=None, config_path=None, url=None, port=None, username=None, password=None):
    ...
    if client is not None:
        ...
        if c is not None:
            ...
            raise Exception(
                f"Could not connect to bot [{url}:{port}]: Check that http://{url}:{port}/api/v1/ping works in a browser, and check any firewall settings.")
    else:
...
def make_layout(exclude_charts=False, include_sysinfo=False, include_candle_info=False, include_tag_summary=False,
                side_panel_minimum_size=114, num_days_daily=5) -> Layout:
    """Define the layout."""
    ...
    if exclude_charts:
        ...
        if include_sysinfo:
            if include_candle_info:
                layout["right_side"].split(Layout(name="chart1", ratio=2), Layout(name="sys_info"),
...
def make_candle_info_layout(exclude_charts=False, include_sysinfo=False, include_tag_summary=False,
                            side_panel_minimum_size=114, num_days_daily=5) -> Layout:
    """Define the layout."""
    layout = Layout(name="root")
    ...
    for n, client in client_dict.items():
        ...
        for tag, trades in trades_by_tag.items():
            ...
            for t in trades:
                ...
                t_profit += profit
                tdur = (datetime.strptime(t['close_date'], dfmt) - datetime.strptime(t['open_date'],
                                                                                     dfmt)).total_seconds()
...
def trades_summary(client_dict) -> Table:
    ...
    table.columns[
        3].footer = f"[red]{round(all_open_profit, 2)}" if all_open_profit <= 0 else f"[green]{round(all_open_profit, 2)}"
...
def open_trades_table(client_dict) -> Table:
    ...
    for n, client in client_dict.items():
        ...
        for t in trades:
            ...
            pairstr = t['pair'] + (
                '*' if (t['open_order_id'] is not None and t['close_rate_requested'] is None) else '') + (
            ...
            if 'buy_tag' in t.keys():
                table.add_row(
                    ...
                    f"[red]{round(t['profit_abs'], 2)}" if t[
                                                               'profit_abs'] < 0 else f"[green]{round(t['profit_abs'], 2)}",
    ...
    for n, client in client_dict.items():
        ...
        for day in t['data']:
            if day['date'] not in dailydict.keys():
                dailydict[day['date']] = [day['date'], f"{fear[day['date']]}",
...
def pair_chart(basic_chart, height=20, width=120, limit=None, timeframe=None, basic_symbols=False):
    ...
    return (
    chart_config['current_pair'], basic_chart.get_chart_str(height=height, width=width, basic_symbols=basic_symbols))
...
def main():
    ...
    parser.add_argument("-k", "--include_candle_info", action="store_true",
                        help="Include 5m candle information. Default: False")
    parser.add_argument("-o", "--include_tag_summary", action="store_true",
        ...
        if args.yaml:
            ...
            for s in args.servers:
                try:
                    if config is not None:
                        ...
                        name, client = setup_client(name=s['name'], url=s['ip'], port=s['port'], username=s['username'],
                                                    password=s['password'])
            ...
            for s in slist:
                ...
                if m:
                    ...
                    try:
                        if config is not None:
                            name, client = setup_client(name=botname, url=url, port=port, username=suser,
                                                        password=spass, config_path=config)
                        else:
                            name, client = setup_client(name=botname, url=url, port=port, username=suser,
                                                        password=spass)
                        client_dict[name] = client
    ...
    if args.exclude_charts and args.include_candle_info:
        layout = make_candle_info_layout(exclude_charts=args.exclude_charts, include_sysinfo=args.include_sysinfo,
                                         ...
                                         side_panel_minimum_size=side_panel_minimum_size, num_days_daily=num_days_daily)
    else:
        layout = make_layout(exclude_charts=args.exclude_charts, include_sysinfo=args.include_sysinfo,
                             include_candle_info=args.include_candle_info, include_tag_summary=args.include_tag_summary,
                             side_panel_minimum_size=side_panel_minimum_size, num_days_daily=num_days_daily)
    ...
    if args.include_sysinfo:
        layout['sys_info'].update(
            Panel(Status("Loading...", spinner="line"), title="[b]System Information", border_style="magenta"))

    if args.include_candle_info:
        layout['candle_info'].update(
            Panel(Status("Loading...", spinner="line"), title="[b]Candle Information", border_style="cyan"))

    if args.include_tag_summary:
        layout['tag_summary'].update(
            Panel(Status("Loading...", spinner="line"), title="[b]Entry Tag Summary", border_style="white"))
                ...
                try:
                    ...
                    if (do_info_panels_update):
                        ...
                        layout["summary"].update(
                            Panel(trades_summary(client_dict), title="Trades Summary", border_style="red",
                                  height=7 + len(client_dict.items())))

                        layout["daily"].update(
                            Panel(daily_profit_table(client_dict, num_days_daily), title="Daily Profit",
                                  border_style="yellow", height=(num_days_daily + 6)))
                        layout["closed"].update(
                            Panel(closed_trades_table(client_dict, all_closed_trades, num_closed_trades),
                                  title="Closed Trades", border_style="blue"))

                        if not args.exclude_charts:
                            spc = pair_chart(pc, height=ch - 4, width=cw, limit=cw,
                            ...
                            if not args.include_sysinfo:
                                ppc = profit_chart(pc, all_closed_trades[chart_config['current_summary']],
                                                   height=ch - 4, width=cw, basic_symbols=args.basic_symbols)
                                layout["chart2"].update(
                                    Panel(ppc, title=f"{chart_config['current_summary']} Cumulative Profit"))
                            else:
                                if args.include_candle_info:
                                    layout["candle_info"].update(
                                        Panel(tradeinfo(client_dict, all_closed_trades, indicators),
                                              title="Recent Buy Info", border_style="cyan"))
                        else:
                            if args.include_candle_info:
                                layout["candle_info"].update(
                                    Panel(tradeinfo(client_dict, all_closed_trades, indicators),
                                          title="[b]Candle Information", border_style="cyan"))

                    layout["open"].update(
                        Panel(open_trades_table(client_dict), title="Open Trades", border_style="green"))
